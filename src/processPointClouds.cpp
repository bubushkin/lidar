...
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::SeparateClouds(pcl::PointIndices::Ptr inliers, typename pcl::PointCloud<PointT>::Ptr cloud) 
{
  ...
  typename pcl::PointCloud<PointT>::Ptr pobstacle{new pcl::PointCloud<PointT>()};
  typename pcl::PointCloud<PointT>::Ptr pplane{new pcl::PointCloud<PointT>()};

  pcl::ExtractIndices<PointT> extract;
  for(auto idx : inliers->indices){
      pplane->points.push_back(cloud->points[idx]);
  }

  extract.setInputCloud(cloud);
  extract.setIndices(inliers);
  extract.setNegative(true);
  extract.filter(*pobstacle);
  

    std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> segResult(pplane, pobstacle);
    return segResult;
}
